...
const RoomCard = ({ id, name, time, number, color }: RoomCardProps ) => (
    <div className="flex flex-col bg-gray-100 border border-gray-400 rounded-md">
        <div className={`px-3 py-1 font-bold text-center text-black ${color || 'bg-gray-300'} rounded-t-md`}>
            방 {id}
        </div>
        <div className="flex flex-col items-center justify-center flex-grow p-2 space-y-2">
            ...
            <div className="flex items-center justify-center w-full h-16 text-4xl font-bold bg-white border border-gray-300 rounded">
                {number}
            </div>
        </div>
    </div>
...
// WaitingList 컴포넌트의 props 타입을 정의
    ...
    return (
        <div className="grid grid-cols-4 gap-1">
            {data.flat().map((item, index) => (
                <div key={index} className="flex items-center justify-center h-11 text-xs bg-white border border-gray-400 md:text-sm">
                    {item}
                </div>
            ))}
        </div>
            ...
            <header className="bg-gray-800 text-white shadow-md">
                <div className="container mx-auto flex justify-between items-center p-3">
                    ...
                    <nav className="flex items-center space-x-6">
                        ...
                        <a href="#" className="text-sm hover:text-gray-300">Logout</a>
                    </nav>
                </div>
            </header>
                ...
                <div className="container p-4 mx-auto bg-white shadow-lg" style={{ borderTop: '3px solid #F97316' }}>
                    ...
                    <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
                        ...
                        <div className="xl:col-span-10">
                            ...
                            {/* Junior and Dream Room Area */}
                            <div className="grid grid-cols-10 gap-3">
                                {/* Junior Cards take 6 columns */}
                                <div className="grid grid-cols-6 col-span-6 gap-3">
                                    {juniorData.map(jr => (
                                        <JuniorCard color={undefined} key={jr.id} {...jr} />
                                    ))}
                                </div>

                                {/* Dream Room & Legend take 4 columns */}
                        ...
                        <div className="flex flex-col space-y-4 xl:col-span-2">
                            ...
                            {/* 2025.09.03 - 유치부는 현재 모두 비활성화 상태, 따라서, 유치부 연습 대기는 비활성화 해도 상관없을 것으로 보임.  */}
                            <WaitingList title="유치부 연습 대기" count={5} color="bg-pink-500" />
                            <button className="px-4 py-6 text-2xl font-bold text-white bg-orange-500 rounded-lg hover:bg-orange-600">
                                입실
                            </button>
                        </div>
                    </div>
